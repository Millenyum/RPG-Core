...
public class CustomEntityCaveSpider extends EntitySpider{
	
	private double speed = 0.7, maxHealth = 20D, knockbackRes = 0D, followRange = 32D, strength = 2D;
	public CustomEntityCaveSpider(World w, String s){
		this(w);
    ...
    public static CustomEntityCaveSpider spawn(Location pos){
    	return CustomEntityCaveSpider.spawn(pos, "");
    }

    public static CustomEntityCaveSpider spawn(Location pos, String name){
    	return CustomEntityCaveSpider.spawn(((CraftWorld)pos.getWorld()).getHandle(), pos.getX(), pos.getY(), pos.getZ(), pos.getYaw(), pos.getPitch(), name);
    }
    
    public static CustomEntityCaveSpider spawn(World nmsWorld, double x, double y, double z, float yaw, float pitch, String name){
    	CustomEntityCaveSpider cez = new CustomEntityCaveSpider(nmsWorld, name);
