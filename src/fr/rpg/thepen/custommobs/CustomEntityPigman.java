...
public class CustomEntityPigman extends EntityPigZombie{
	
	private double speed = 0.7, maxHealth = 20D, knockbackRes = 0D, followRange = 32D, strength = 2D;
	public CustomEntityPigman(World w, String s){
		this(w);
    ...
    public CustomEntityPigman(World world){
        super(world);
    }
    
    public static CustomEntityPigman spawn(Location pos){
    	return CustomEntityPigman.spawn(pos, "");
    }

    public static CustomEntityPigman spawn(Location pos, String name){
    	return CustomEntityPigman.spawn(((CraftWorld)pos.getWorld()).getHandle(), pos.getX(), pos.getY(), pos.getZ(), pos.getYaw(), pos.getPitch(), name);
    }
    
    public static CustomEntityPigman spawn(World nmsWorld, double x, double y, double z, float yaw, float pitch, String name){
    	CustomEntityPigman cez = new CustomEntityPigman(nmsWorld, name);
